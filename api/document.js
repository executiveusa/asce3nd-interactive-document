...
const ES_EXTRA = {
  ...
  'What was she seeing in young people and the community?': '¿Qué veía ella en los jóvenes y la comunidad?',
  'Where does her belief in mentorship come from?': '¿De dónde viene su convicción sobre la mentoría?',
  'Who believed in her — or who did she wish had?': '¿Quién creyó en ella o a quién le hubiera gustado que creyera?',
  'What should a young person feel when they encounter Asc3nd?': '¿Qué debería sentir un joven al encontrarse con Asc3nd?',
  'What should parents understand about Asc3nd?': '¿Qué deberían entender los padres sobre Asc3nd?',
  'Why begin with Community Cuts for Kids?': '¿Por qué comenzar con Community Cuts for Kids?',
  'Finish the sentence: "Asc3nd exists because…"': 'Completa la frase: «Asc3nd existe porque…»',
  'Elisha — Individual Interview Questions': 'Elisha — Preguntas para entrevista individual',
  'What made this personal for him?': '¿Qué hizo que esto fuera personal para él?',
  'What did he see missing for young people?': '¿Qué veía él que faltaba para los jóvenes?',
  'What does "someone believing in you" mean to him?': '¿Qué significa para él que "alguien crea en ti"?',
  ...
  'What does mentorship look like in practice?': '¿Cómo se ve la mentoría en la práctica?',
  'Finish the sentence: "Asc3nd exists because…"': 'Completa la frase: «Asc3nd existe porque…»',
  ...
  // ── Phase 04/08/09 step titles and key strings ──
  'Truth Ledger': 'Libro de verdades',
  'Responsibilities': 'Responsabilidades',
  'Responsibilities & Accountability': 'Responsabilidades y rendición de cuentas',
  'Run-of-Show': 'Cronograma del evento',
  'Event Run-of-Show': 'Cronograma del evento',
  'Operations': 'Operaciones',
  'Operations — Supplies, Food, Venue, Safety, Consent': 'Operaciones — Suministros, Comida, Sede, Seguridad, Consentimiento',
  "Friday's Interview — Prep": 'Preparación de la entrevista del viernes',
  "Friday's Interview — Director's Prep": 'Preparación del director para la entrevista del viernes',
  'Chapter 2 — THE EVENT': 'Capítulo 2 — EL EVENTO',
  'PLANNING DATA — not live registrations.': 'DATOS DE PLANIFICACIÓN — no son inscripciones reales.',
  'Demand signal — live RSVP': 'Señal de demanda — RSVP en vivo',
  'Loading live RSVP counts…': 'Cargando conteos de RSVP en vivo…',
  'Refresh': 'Actualizar',
  ...
  'PROJECTED TO FILL — consider adding a barber': 'PROYECCIÓN: SE LLENARÁ — considera añadir un barbero',
  'Tracking toward capacity': 'Avanza hacia la capacidad',
  'Last synced': 'Última sincronización',
  'Offline — showing cached data. Live counts will reload when connection returns.': 'Sin conexión — mostrando datos en caché. Los conteos se actualizarán al volver la conexión.',
  'Generate camera handoff': 'Generar documento para el camarógrafo',
  'You\'re about to tell the story of why Asc3nd exists.': 'Estás a punto de contar la historia de por qué existe Asc3nd.',
  'Run order': 'Orden del programa',
  'Story spine — where each answer lands': 'Columna vertebral — dónde aterriza cada respuesta',
  'What to bring': 'Qué traer',
  'Technical checklist (Anton)': 'Lista técnica (Anton)',
  'Consent confirmed': 'Consentimiento confirmado'
};
...
async function saveToCloud(showToast){if(cloudSaving)return false;cloudSaving=true;syncState='SAVING';updateSaveMsg();saveLocal();var el=document.getElementById('save-msg');if(el)el.textContent='Saving to cloud…';try{var res=await fetch(SUPABASE_RPC+'save_submission',{method:'POST',headers:cloudHeaders(),body:JSON.stringify({p_project_id:PROJECT_ID,p_device_id:getDeviceId(),p_payload:{workbook:state,saved_at:new Date().toISOString()}})});if(!res.ok)throw new Error('Cloud save failed: '+res.status);syncState='SYNCED';if(el)el.textContent='Saved to cloud';if(showToast)toast('Submitted and saved to cloud');
try{fetch('https://api.thepaulieffect.com/mailer/webhook/submission',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device_id:getDeviceId(),payload:{workbook:state,saved_at:new Date().toISOString()}})}).catch(function(){});}catch(e){}
return true;}catch(err){console.error(err);syncState='SYNC_FAILED';if(el)el.textContent='Saved locally — cloud unavailable';if(showToast)toast('Saved locally. Cloud save failed.');return false;}finally{cloudSaving=false;updateSaveMsg();}}
async function loadFromCloud(){try{var res=await fetch(SUPABASE_RPC+'load_latest_submission',{method:'POST',headers:cloudHeaders(),body:JSON.stringify({})});if(!res.ok){syncState='OFFLINE_CACHED';updateSaveMsg();return;}var row=await res.json();var payload=row&&row.payload;if(payload&&payload.workbook){var cloudWb=payload.workbook;var cloudTs=cloudWb.lastSaved?new Date(cloudWb.lastSaved).getTime():0;var localTs=state.lastSaved?new Date(state.lastSaved).getTime():0;var liveEdm=state.chapter2&&state.chapter2.eventDayMode?Object.assign({},state.chapter2.eventDayMode):null;if(cloudTs>localTs){state=Object.assign(blank(),cloudWb);if(liveEdm&&state.chapter2)state.chapter2.eventDayMode=liveEdm;syncState='SYNCED';saveLocal();if(started)renderStep();}else if(cloudTs===localTs&&cloudTs>0){syncState=(JSON.stringify(cloudWb)===JSON.stringify(state))?'SYNCED':'CONFLICT_REQUIRES_REVIEW';}else{syncState='SYNCED';saveToCloud(false);}}}catch(err){console.warn('Cloud load unavailable',err);syncState='OFFLINE_CACHED';}finally{updateSaveMsg();}}
